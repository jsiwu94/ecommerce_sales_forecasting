...
length(unique(tsdf$order_purchase_timestamp))*365
```
...
#merged_df['avg_sales_diff30'] <- diff(ts(merged_df$sales_volume), differences = 30)
#merged_df['avg_sales_diff14'] <- diff(ts(merged_df$sales_volume), differences = 14)
...
#TSLM
fit.tslm <- traintsb %>% 
 model(TSLM(sales_volume ~ month + trend() + season("week") + fourier(period = "month",3)))
fit.tslm %>% report()
...
```{r, warning=FALSE}
fit <- traintsb %>% 
        model(tuned_arima = ARIMA(sales_volume ~ pdq(4, 1, 2) + PDQ(0, 1, 1, period = 7)),
              tuned_lm_arima =  ARIMA(sales_volume ~ month + weekday + holiday + day + trend() + fourier(period = "week",1)),
              ets = ETS(sales_volume),
              tslm = TSLM(sales_volume ~ month + trend() + season("week") + fourier(period = "month",3))
              #,ensmbl_wt = combination_model(ARIMA(sales_volume ~ 0 + pdq(4, 1, 2) + PDQ(0, 1, 1, period = 7))
              #                             ,ETS(sales_volume), cmbn_args = list(weights = "inv_var"))
)
fit %>% report()
```
