...
```{r}
fcst_actual_df <- right_join(tsdf80,fcst, by=c("order_purchase_timestamp"="ds","product_category_name_english"), copy=FALSE)
fc_act_noNA <- na.omit(fcst_actual_df) %>% mutate(lost_sales = ifelse(yhat-sales_volume < 0,0,abs(yhat-sales_volume)),
                                             lost_revenue = ifelse(yhat-sales_volume < 0,0,abs(yhat-sales_volume))*revenue)
sum(fc_act_noNA$lost_revenue)/nrow(fc_act_noNA)*365 #the average annual lost revenue saved
```
...
```{r}
#final_df %>% mutate(pred = map(model, make_prediction))
tempdfdf <- final_df %>% mutate(pred = map(model, make_prediction))
fcst_unnest <- unnest(tempdfdf,pred)
fcst_data <- fcst_unnest %>% select(product_category_name_english,ds,yhat) %>% group_by(ds) %>% summarise(forecast = sum(yhat))
```
