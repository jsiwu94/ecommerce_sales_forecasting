...
monthtemp <- tsdf80f[tsdf80f$order_purchase_timestamp < '2018-01-01',] %>% group_by(month) %>% summarise(sales_volume= sum(sales_volume)) 
...
```{r}
fit.arlm <- traintsb %>% 
        model(SARIMAX = ARIMA(sales_volume ~ month + weekday + holiday + day + trend() + fourier(period = "week",1)
...
fc.arlm %>% 
  autoplot(alltsb, level =NULL) +
  ggtitle("Forecasts vs Actual for Top 80 Sales Products - SARIMAX") +
  xlab("Year") +
  guides(colour = guide_legend(title = "Forecast"))
```
...
fit <- traintsb %>% 
        model(sarima = ARIMA(sales_volume ~ 0 + pdq(4, 1, 2) + PDQ(0, 1, 1, period = 7)),
              sarimax =  ARIMA(sales_volume ~ month + weekday + holiday + day + trend() + fourier(period = "week",1)+
                                fourier(period = "month",3) + fourier(period = "year",5)),
...
# Making the Final Prediction by Product Category
```{r, warning=FALSE,message=FALSE}
...
make_prediction <- function(model) {
  ft <- data.frame(ds =seq(as.Date("2017-01-05"),as.Date("2018-07-31")+60, by="days"))
  prediction <- predict(model, ft)
  return(prediction)
  }


final_df <- proddf %>%  
           nest(-product_category_name_english) %>%
           mutate(model= map(data, make_forecast)) %>%
final_df <- final_df %>% mutate(pred = map(model, make_prediction))
#saveRDS(final_df, "model_nested_df.rds")


fcst_unnest <- unnest(final_df,pred)
fcst_data <- fcst_unnest %>% select(product_category_name_english,ds,yhat)
```

# Joining back to The "Actual" Data to Create the Comparison Plot
```{r}
fcst_actual_df <- right_join(tsdf80,fcst_data, by=c("order_purchase_timestamp"="ds","product_category_name_english"), copy=FALSE)


fcst_actual_df %>%
  group_by(order_purchase_timestamp) %>% summarise(actual = sum(sales_volume),
                             forecast = sum(yhat)) %>%
                                  gather(key = "type", value = "value", - order_purchase_timestamp) %>%
  ggplot(aes(order_purchase_timestamp,value, color = type)) +
  geom_line() + 
  ggtitle("Sales Volume Forecast vs Actual from 2017 to 2018") +
  scale_x_date(date_labels = "%m-%Y") +
  scale_color_manual(values=c("#213ee2", "#06d997")) +
  theme(plot.title = element_text(size = 22, face = "bold"))
```
...
paste("estimated annual lost sales from this model :",abs(sum(fcst_actual_top$lost_sales)/nrow(fcst_actual_top)*365))
```
